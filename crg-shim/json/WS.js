var WS = {
  ...
  Connect: function (callback) {
    if (!WS._started) {
      ...
      $.getScript('/external/signalr/signalr.min.js', function () {
        WS._connect();
      });
    }
  },
  ...
  _connect: function () {
    WS._connection = new signalR.HubConnectionBuilder()
      .withUrl('/api/hubs/carolina')
      .withAutomaticReconnect()
      .build();

    WS._connection.on('StateUpdated', function (state) {
      WS._processUpdate(state);
    });

    WS._connection.onreconnecting(function () {
      WS._Connected = false;
      WS._connectionStatus.attr('status', 'error').text('Reconnecting...');
    ...
    WS._connection.onreconnected(function () {
      WS._Connected = true;
      WS._connectionStatus.attr('status', 'loading').text('Loading...');
      if (WS._registerOnConnect.length > 0) {
        WS._connection.invoke('Register', WS._registerOnConnect).then(function () {
          WS._connectionStatus.attr('status', 'ready').text('');
        });
      }
    });

    WS._connection.onclose(function () {
      WS._Connected = false;
      WS._connectionStatus.attr('status', 'error').text('Not connected');
    });

    WS._connection.start()
      .then(function () {
        WS._Connected = true;
        WS._connectionStatus.attr('status', 'loading').text('Loading...');
        if (WS._registerOnConnect.length > 0) {
          WS._connection.invoke('Register', WS._registerOnConnect).then(function () {
            WS._connectionStatus.attr('status', 'ready').text('');
            $.each(WS._afterLoadCallbacks, function (idx, func) { func(); });
      ...
      .catch(function (err) {
        console.error('WS', 'SignalR connection error:', err);
        WS._connectionStatus.attr('status', 'error').text('Not connected');
      });
  },
  ...
  Command: function (command, data) {
    console.warn("WS.Command is not implemented yet");
  },

  Set: function (key, value, flag) {
    console.warn("WS.Set is not implemented yet");
  },
  ...
  Register: function (paths, options, isPreRegister) {
    ...
    if (paths.length) {
      ...
      if (WS._Connected) {
        var pathsToSend = isPreRegister ? WS._registerOnConnect : paths;
        WS._connection.invoke('Register', pathsToSend).then(function () {
          if (isPreRegister) {
            WS._connectionStatus.attr('status', 'ready').text('');
            $.each(WS._afterLoadCallbacks, function (idx, func) { func(); });
            WS._afterLoadCallbacks = [];
