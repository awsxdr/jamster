...
public sealed class JamClockSet(Guid7 id, JamClockSetBody body) : Event<JamClockSetBody>(id, body);
public sealed record JamClockSetBody(int SecondsRemaining);

public sealed class LineupClockSet(Guid7 id, LineupClockSetBody body) : Event<LineupClockSetBody>(id, body);
public sealed record LineupClockSetBody(int SecondsPassed);

public sealed class PeriodClockSet(Guid7 id, PeriodClockSetBody body) : Event<PeriodClockSetBody>(id, body);
public sealed record PeriodClockSetBody(int SecondsRemaining);

public sealed class TimeoutClockSet(Guid7 id, TimeoutClockSetBody body) : Event<TimeoutClockSetBody>(id, body);
