...
export const ScoreSheetJamRow = ({ line, even }: ScoreSheetJamRowProps) => {
    ...
    return (
        <>
            ...
            <div contentEditable className={cn("col-start-2", numberCellClass, rowColorClass)}>{line.jammerNumber}</div>
            ...
            { Array.from(new Array(9)).map((_, i) => (
                <div contentEditable key={`trip-${i+2}`} className={cn(`col-start-${i + 8}`, numberCellClass, rowColorClass)}>{line.trips[i]?.score}</div>
            ))}
